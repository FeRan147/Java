	...
	public static void main(String [] args) {
		...
		if(firstNumber.matches(regexStr) == true && secondNumber.matches(regexStr) == true) {
			...
			if(firstNumber.matches(regexStr) == false && secondNumber.matches(regexStr) == false) {
				System.out.println("Обе введенные записи не является числами. Деление невозможно!");
			} else {
				if(firstNumber.matches(regexStr) == false) {
					System.out.println("Первая запись не является числом. Деление невозможно!");
				} else {
					System.out.println("Вторая запись не является числом. Деление невозможно!");
				}
			}
		}
		inputStr.close();
